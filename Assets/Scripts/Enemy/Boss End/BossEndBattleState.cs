...
public class BossEndBattleState : EnemyState
{
    protected Enemy_BossEnd enemy;

    private bool canAttackPlayer;

    public BossEndBattleState(Enemy _enemyBase, EnemyStateMachine _enemyStateMachine, string _animateBoolName,Enemy_BossEnd _enemy) : base(_enemyBase, _enemyStateMachine, _animateBoolName)
    {
        enemy=_enemy;
    }
    ...
    public override void Update()
    {
        ...
        if(Vector2.Distance(enemy.transform.position,player.transform.position) <= enemy.attackDistance){
            if(CanAttack()&&enemy.IsPlayerDetected()){
                if(enemy.CanUseBossEndSkill1()){
                    stateMachine.ChangeState(enemy.attackState1);
                }
                else if(enemy.CanUseBossEndSkill2()){

                    enemy.Attack3();
                }
                else if(enemy.CanUseBossEndSkill3()){
                    stateMachine.ChangeState(enemy.attackState2);
                    enemy.canUseSkill3=false;
                }else{
