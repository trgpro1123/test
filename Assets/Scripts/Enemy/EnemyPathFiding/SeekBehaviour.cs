...
public class SeekBehaviour : SteeringBehaviour
{
    ...
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, AIData aiData)
    {
        ...
        else if (customTargetPosition.HasValue)
        {
            targetPositionCached = customTargetPosition.Value;
        }
        ...
        //     return (danger, interest);
        // }
        if (Vector2.Distance(transform.position, targetPositionCached)<=enemy.attackDistance)
        {
            reachedLastTarget = true;
            aiData.currentTarget = null;
            customTargetPosition = null;
    ...
    public void SetCustomTargetPosition(Vector2 position)
    {
        customTargetPosition = position;
        reachedLastTarget = false; // Đảm bảo AI biết rằng có một mục tiêu mới cần tới
    }
